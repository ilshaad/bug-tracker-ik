...
export default function DisplayTicket({ ticket }: Props) {
  ...
  return (
    <>
      ...
      <h3 className={`viewTicket-headings viewTicket-text-secondary`}>
        Priority:
      </h3>
      {(() => {
        if (priority === "High") {
          return <p className="viewTicket-danger fw-bold">High</p>;
      ...
      {status === "Resolved" ? (
        ...
        <p className="viewTicket-status-pending fw-bold">Pending</p>
      )}
      ...
      {assigned_user === "Unassigned" ? (
        <p className="viewTicket-danger fw-bold">Unassigned</p>
      ) : (
        <p>{assigned_user}</p>
      )}
